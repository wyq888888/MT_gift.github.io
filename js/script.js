...
//随机文字烟花内容
const randomWords = ["马腾", "新年快乐", "2026", "心想事成", "万事如意", "2026", "岁岁平安", "马腾", "马腾", "恭喜发财", "2026", "马腾", "2026", "马腾", "马腾", "事业有成", "蒸蒸日上", "马腾", "马腾", "马腾", "四季平安", "马腾", "马腾"];
const wordDotsMap = {};
randomWords.forEach((word) => {
	wordDotsMap[word] = MyMath.literalLattice(word, 4, "Gabriola,华文琥珀", "90px");
});
...
document.addEventListener("DOMContentLoaded", function () {
	...
	// canvasContainer.style.backgroundImage = "url('C:\\Users\\买房 人生大计\\Desktop\\6fec9b83ffe4ef9930173a6acd4d28e7.jpg')";
	...
	var musicIconUse = musicBtn && musicBtn.querySelector("use");
	function updateMusicIcon(playing) {
		if (musicIconUse) {
			var ref = playing ? "#icon-sound-on" : "#icon-sound-off";
			musicIconUse.setAttribute("href", ref);
			musicIconUse.setAttribute("xlink:href", ref);
		}
	}
	...
	if (bgmEl && musicBtn) {
		try {
			bgmEl.loop = true;
			bgmEl.volume = 0.5; // 可根据需要调整音量 0.0 - 1.0
		} catch (e) {}
		...
		document.addEventListener("visibilitychange", function () {
			if (document.hidden && !bgmEl.paused) {
				bgmEl.pause();
				updateMusicIcon(false);
			}
		});

		// 首次用户交互时尝试自动播放（符合浏览器策略）
		function attemptAutoPlay() {
			...
			if (p && typeof p.then === "function") {
				p.then(function () {
					...
					// 若因策略失败则保持静音，等待用户点击音乐按钮
				});
			} else {
				updateMusicIcon(true);
			}
		}
		window.addEventListener("pointerdown", attemptAutoPlay, { once: true, capture: true });
		window.addEventListener("keydown", attemptAutoPlay, { once: true, capture: true });
	}
});
